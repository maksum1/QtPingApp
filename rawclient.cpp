...
RawClient::RawClient(): packet_size(32), m_ttl(30), MAX_PING_DATA_SIZE(1024),
...
int RawClient::allocate_buffers(ICMPHeader*& send_buf, IPHeader*& recv_buf,
        int packet_size)
{
...
int RawClient::setup_for_ping(char* host, int ttl, SOCKET& sd, sockaddr_in& dest)
{
    // Create the socket
...
void RawClient::init_ping_packet(ICMPHeader* icmp_hdr, int packet_size, int seq_no)
{
...
int RawClient::send_ping(SOCKET sd, const sockaddr_in& dest, ICMPHeader* send_buf,
        int packet_size)
{
...
int RawClient::recv_ping(SOCKET sd, sockaddr_in& source, IPHeader* recv_buf,
        int packet_size)
{
...
int RawClient::decode_reply(IPHeader* reply, int bytes, sockaddr_in* from)
{
